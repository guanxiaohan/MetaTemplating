...
if __name__ == "__main__":
    # if len(sys.argv) < 2:
    #     print("Usage: python GenerateFromTemplate.py <template_file>")
    #     sys.exit(1)
    ...
    template_file = "./archivePasswordTemplate.txt"  # Example template file
    template = Template.load_template(template_file)
    template.analyze_template()

    print("Analyzed Template:")
    pprint(template.applyingTemplate)

    # print("Directly output")
    # pprint(template.fill_template())

    print("Output with variables")
    test_variables = {
        "appliedFiles": "apf",
        "allPasswords": "pwds",
        "particularPasswords": "allpwds",

    }
    print()
    print(template.fill_template(test_variables))
